...
<div class="wrap wb-admin">
      ...
      <table class="widefat options" style="width: 650px">
		...
		<td class="kma-admin-td"><label><input type="text" style="width:125px;" maxlength="55" value="<?php echo get_option('kma_width'); ?>" name="kma_width" id="kma_width" class="regular-text" />percent</label>
		  <span class="kma_help" title="<?php _e('Complete this input with the width of your choice for the left and righ clickable zones in percent', 'kma'); ?>"></span><br /></td>
	  </tr>
      </table>
        <?php
		if(isset($_POST['submitted']) && $_POST['submitted'] == "yes")
		{
			...
			if(isset($_POST['kma_background_url']) && $_POST['kma_background_url'] != '' && isset($_POST['kma_background_partner_url']) && $_POST['kma_background_partner_url'] != '' && isset($_POST['kma_background_priority']) && $_POST['kma_background_priority'] != ''&& isset($_POST['kma_background_category']) && $_POST['kma_background_category'] != '')
			{
				global $wpdb;
				$table_name = $wpdb->prefix;
	...
	/*			$rows_affected = $wpdb->insert( $table_name, array('BackgroundUrl' => $background_url, 'PartnerUrl' => $background_partner_url, 'Priority' => $background_priority, 'Category' => $background_category), array('%s','%s','%s','%s'));*/
				$rows_affected = $wpdb->query( $wpdb->prepare( "INSERT INTO {$table_name}background_partner ( BackgroundUrl, PartnerUrl, Category, Priority ) VALUES ( %s, %s, %s, %s )", ''.$background_url.'', ''.$background_partner_url.'', ''.$background_category.'', ''.$background_priority.'' ) );
			}
